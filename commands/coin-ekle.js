...
module.exports = {
    name: 'coin-ekle',
    description: 'Bir kullanıcıya coin ekler.',
    async execute(message, args) {
        ...
        if (isNaN(amountToAdd) || amountToAdd <= 0) {
            return message.reply('Lütfen geçerli bir coin miktarı girin.');
        }
        ...
        db.get(`SELECT * FROM users WHERE userId = ?`, [user.id], (err, row) => {
            ...
            let newCoins = row ? row.coins + amountToAdd : amountToAdd;
            let level = row ? row.level : 1;

            updateUserAndRole(message.client, user.id, newCoins, level); 

            db.run(`INSERT OR REPLACE INTO users (userId, coins, level, experience, lastMessageTimestamp, voiceStartTimestamp) VALUES (?, ?, ?, ?, ?, ?)`,
                [user.id, newCoins, level, row ? row.experience : 0, new Date().toISOString(), row ? row.voiceStartTimestamp : null],
                (err) => {
                    if (err) {
                        console.error('Coin eklerken hata:', err.message);
                        return message.reply('Bir hata oluştu.');
                    }
                    console.log(`Eklenen coin: ${user.username} - ${amountToAdd}`);
                    message.channel.send(`${user.username} kullanıcısına ${amountToAdd} coin eklendi.`);
                }
            );
        });
    },
